...
const std::pair<std::string, std::string> sample_message_list[sample_size] = {
    mess_and_hash("", "e3b0c44298fc1c149afbf4c"
                     "8996fb92427ae41e4649b934ca495991b7852b855"),
    mess_and_hash("The quick brown fox jumps over the lazy dog",
            "d7a8fbb307d7809469ca9abcb0082e4f8d"
            "5651e46d3cdb762d02d0bf37c9e592"),
    mess_and_hash("The quick brown fox jumps over the lazy dog.",
            "ef537f25c895bfa782526529a9b63d97aa6"
            "31564d5d789c2b765448c8635fb6c"),
    mess_and_hash("For this sample, this 63-byte"
                  " string will be used as input data",
            "f08a78cbbaee082b052ae0708f32fa1e50c5c42"
            "1aa772ba5dbb406a2ea6be342"),
    mess_and_hash("abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq",
            "248d6a61d20638b8e5c026930c3e6"
            "039a33ce45964ff2167f6ecedd419db06c1"),
    mess_and_hash("This is exactly 64 bytes long, "
                  "not counting the terminating byte",
            "ab64eff7e88e2e46165e29f2bce41826bd4c7b"
            "3552f6b382a9e7d3af47c245f8"),
    mess_and_hash("This is exactly 64 bytes long, not counting the terminati",
            "46db250ef6d667908de17333c2534"
            "3778f495d7a8010b9cfa2af97940772e8cd"),
    mess_and_hash("This is exactly 64 bytes "
                  "long, not counting the terminatin",
            "af38fc14dbbbcc6cd4c9cc73988e1b"
            "08373b4e6b04ba61b41f999731185b51af"),
    mess_and_hash("This is exactly 64 bytes long"
                  ", not counting the terminating",
            "f778b361f650cdd9981ca13adb77f"
            "26b8419a407b3938fc54b14e9971045fa9d"),
    mess_and_hash("This is exactly 64 bytes"
                  " long, not counting the terminating b",
            "9aa72d139c7d7e5a35ea525e2ba67"
            "04163555ba647927765a61ccbf12ec60479")
};

void test(){
    for (std::size_t i = 0; i < sample_size; ++i){
        std::string src_str = sample_message_list[i].first;
        std::cout << "src_str: " << src_str  <<
        " size: " << src_str.length() << std::endl;
        ...
        {
            std::vector<unsigned char> hash(picosha2::k_digest_size);
            picosha2::hash256(src_vect.begin(),
                    src_vect.end(), hash.begin(), hash.end());
            PICOSHA2_CHECK_EQUAL_BYTES(ans, hash);
        }
        {
            std::vector<unsigned char> hash(picosha2::k_digest_size);
        ...
        }
        {
            ...
            std::vector<unsigned char> hash(picosha2::k_digest_size);
            picosha2::hash256(src.begin(),
                    src.end(), hash.begin(), hash.end());
            PICOSHA2_CHECK_EQUAL_BYTES(ans, hash);
        }
    }
    {
